...
int pino_sct = A0;          // Analog pin for current sensor (not used in this code)
...
void loop() {
  ...
  if (looptime - time2 >= 1000) { // Update every minute
    ...
    Lora_Transmitter.print("/exec?corrente=" + co + "&irradiancia=" + in); // Send data to ESP8266
    Serial.println("##send##");
    Serial.print("/exec?corrente=" + co + "&irradiancia=" + in + "\0");
    debug();
  }
}
